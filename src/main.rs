...
impl ChainRepl {
    ...
    fn process_action(&mut self, msg: ChainReplCommand, event_loop: &mut mio::EventLoop<ChainRepl>) {
        trace!("{:p}; got {:?}", self, msg);
        match msg {
            ChainReplCommand::Broadcast(s) => {
            ...
            ChainReplCommand::NewConnection(socket) => {
...
impl Connection {
    ...
    fn register(&self, event_loop: &mut mio::EventLoop<ChainRepl>, token: mio::Token) {
    ...
    fn process_rules(&mut self, event_loop: &mut mio::EventLoop<ChainRepl>,
        to_parent: &mut VecDeque<ChainReplCommand>) {
    ...
    fn process_buffer(&mut self, to_parent: &mut VecDeque<ChainReplCommand>) {
        let mut prev = 0;
        ...
        for n in self.read_buf.iter().enumerate()
            ...
            let cmd = ChainReplCommand::Broadcast(s);
            info!("Send! {:?}", cmd);
    ...
    fn reregister(&mut self, event_loop: &mut mio::EventLoop<ChainRepl>) {
    ...
    fn process_rules(&mut self, event_loop: &mut mio::EventLoop<ChainRepl>,
        ...
        if self.sock_status.is_readable() {
            ...
            match self.listener.accept() {
                Ok(Some(socket)) => {
                    let cmd = ChainReplCommand::NewConnection(socket);
                    to_parent.push_back(cmd);
                }
                Ok(None) => {
...
fn main() {
    ...
    info!("running chain-repl-test listener at: {:?}", address);
