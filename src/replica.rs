...
#[cfg(test)]
pub mod test {
    use data::{Buf, OpResp, Operation, PeerMsg, ReplicationMessage, Seqno};
    use config::Epoch;
    use quickcheck::{self, Arbitrary, Gen, TestResult};
    use replica::{Log, Outputs, ReplModel};
    use std::sync::mpsc::channel;
    use replication_log::test::{TestLog, VecLog, hash};
                    ...
                    |t, sim, state| {
                        if t < 3 {
                            sim.client_operation(state,
                                                 format!("hello_world at {}", t).into_bytes())
                        }
                    });
        sim.validate_logs();
